...
/**
 * MainController is responsible for managing the main application view, It
 * handles a menu bar with "Log Out" and "Exit" as menu options.
 *
 * @author Aitor
 */
public class MainController implements Initializable {
    ...
    private User user = new User("rabioster@ge", "abcd*123", "Asier", "walk00", true, "london", 48965);

    /**
     * Initializes the MainController. Sets event handlers for the "Log Out" and
     * "Exit" menu items.
     *
    ...
    public void initStage(Parent root, User user) {
        ...
        l_logIn.setText(user.getUsername());
        /*l_name.setText(user.getName());
        l_city.setText(user.getCity());
        l_street.setText(user.getStreet());
        l_zip.setText(Integer.toString(user.getZip()));*/

        if (user.getActive()) {
            l_active.setSelected(true);
        }
        stage.show();
     ...
     * Displays a confirmation dialog to the user before closing the
     * application. If the user confirms, the application exits; otherwise, the
     * event is consumed.
     *
    ...
    /**
     * Handles the "Log Out" action, triggered when the "Log Out" menu item is
     * selected. Loads the SignIn view and switches the current stage to display
     * the SignIn scene.
     *
